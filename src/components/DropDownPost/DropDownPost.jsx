...
const { mutate}= useMutation({
  ...
  onSuccess:()=>{
    ...
    querClinet.invalidateQueries({queryKey:["userPosts"]});
...
function updatePost() {
  return axios.put(`https://route-posts.routemisr.com/posts/${postId}`,createFormDate(),{
    headers:{
...
const{data:delInfo, error:delErr, mutate:delPostFn} = useMutation({
    mutationFn:deletePost,
    onSuccess:()=>{
       querClient.invalidateQueries({queryKey:["getposts"]});
       querClient.invalidateQueries({queryKey:["userPosts"]});
       toast.success("post deleted successfully")
    }

})

console.log("here is the data for del post", delInfo , delErr);


  return (
    <>
 <Dropdown>
      <Button aria-label="Menu" className="bg-white" >
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="oklch(70.7% 0.165 254.624)" className="size-6">
     <path strokeLinecap="round" strokeLinejoin="round" d="M6.75 12a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0ZM12.75 12a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0ZM18.75 12a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Z" />
</svg>

  </Button>
      <Dropdown.Popover>
        <Dropdown.Menu onAction={(key) => key === "edit-file" ? setmodalOpen(true):""}>
          <Dropdown.Item id="edit-file" textValue="Edit file">
             Edit post
          </Dropdown.Item>
          <Dropdown.Item id="delete-file" textValue="Delete file" variant="danger">
            <button className="text-red-500" onClick={delPostFn}>Delete file</button>
          </Dropdown.Item>
        </Dropdown.Menu>
      </Dropdown.Popover>
    </Dropdown>
